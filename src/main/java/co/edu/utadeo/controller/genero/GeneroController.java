...
public class GeneroController extends JFrame {
	
	private JTable table;
	private GeneroMusicalDAO generoMusicalDAO;
	private String[] columnsTable = {"Id","Nombre"};
	/**
	 * Create the frame.
	 */
	public GeneroController() {
		...
		table = new JTable();
		table.setBounds(69, 320, 418, -211);
		panel.add(table);
		...
		btnEditar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				JFrame frame = new GeneroEditController();
                frame.setSize(582,490);
                frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
                frame.setLocationRelativeTo(null);
                frame.setVisible(true);
		...
		JLabel lblNewLabel = new JLabel("The Star Rover - G\u00E9nero Musical");
		lblNewLabel.setBounds(136, 11, 339, 52);
	...
	private void initializeTable(JPanel panel){
		List<GeneroMusical> genero = generoMusicalDAO.findAll();
		Object[][] data = new String[genero.size()][2];
		for(int i=0; i<genero.size(); i++){
			data[i][0] = String.valueOf(genero.get(i).getId());
			data[i][1] = String.valueOf(genero.get(i).getNombre());
		}
