...
public class DisenadorController extends JFrame {
	
	private JTable table;
	private DisenadorDAO disenadorDAO;
	private String[] columnsTable = {"Id","Nombre", "Pasaporte", "pais"};
	...
	public DisenadorController() {
		...
		btnEditar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					 JFrame frame = new DisenadorEditController();
		                frame.setSize(582,490);
		                frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		                frame.setLocationRelativeTo(null);
		                frame.setVisible(true);
		...
		JLabel lblNewLabel = new JLabel("The Star Rover - Dise\u00F1ador");
		lblNewLabel.setBounds(144, 30, 311, 52);
	...
	private void initializeTable(JPanel panel){
		List<Disenador> disenador = disenadorDAO.findAll();
		Object[][] data = new String[disenador.size()][4];
		for(int i=0; i<disenador.size(); i++){
			data[i][0] = String.valueOf(disenador.get(i).getId());
			data[i][1] = String.valueOf(disenador.get(i).getNombre());
			data[i][2] = String.valueOf(disenador.get(i).getPasaporte());
			data[i][3] = String.valueOf(disenador.get(i).getPais().getNombre());
		}
